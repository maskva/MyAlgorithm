...
class Tuple {
public:
    Tuple(KEY_TYPE key, VALUE_TYPE value) {
        this->key = key;
        this->value = value;
    }
    VALUE_TYPE getValue() { return value; };
    void setValue(KEY_TYPE value) { this->value = value; }
public:
    KEY_TYPE key;
    VALUE_TYPE value;
};
...
protected:
    ...
    KEY_TYPE highKey;//high key
    CInternalNode* right;//右节点
...
public:
    ...
    KEY_TYPE GetElement(int i)
    {
        if ((i > 0) && (i <= MAXNUM_DATA)&& m_Tuples[i - 1])
        {
            return m_Tuples[i - 1]->key;
        }
        else
        {
            return INVALID;
    ...
    void SetTuple(int i, Tuple* tuple) {
...
protected:
    ...
    Tuple* m_Tuples[MAXNUM_DATA];//元组指针数组
};
