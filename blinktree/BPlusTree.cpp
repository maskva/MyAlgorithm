...
// 在叶子结点中插入数据
bool CLeafNode::Insert(Tuple* tuple)
{
    int i, j;
    // 如果叶子结点已满，直接返回失败
    ...
    // 找到要插入数据的位置
    for (i = 0;  m_Tuples[i]&&(tuple->key > m_Tuples[i]->key) && (i < m_Count); i++)
    {
    }
...
bool CLeafNode::Delete(KEY_TYPE key)
{
    int i, j;
    bool found = false;
    for (i = 0; i < m_Count; i++)
    {
        if (key == m_Tuples[i]->key)
        {
            found = true;
            break;
        }
...
bool CLeafNode::Combine(CNode* pNode)
{
    ...
    for (int i = 1; i <= pNode->GetCount(); i++)
    {
        this->Insert(new Tuple( pNode->GetElement(i),TEMP));
    }
...
*/
bool BPlusTree::Insert(KEY_TYPE _key, VALUE_TYPE _value)  //
{
    // 检查是否重复插入
    bool found = Search(_key, NULL);
    if (true == found)
    {
        return false;
    }
    ...
    // 查找理想的叶子结点
    CLeafNode* pOldNode = SearchLeafNode(_key);
    ...
    {
        return pOldNode->Insert(new Tuple(_key,_value));
    }
    ...
    if (_key < key)
    {
        pOldNode->Insert(new Tuple(_key,_value));    // 插入原结点
    }
    else
    {
        pNewNode->Insert(new Tuple(_key,_value));    // 插入新结点
    }
...
bool BPlusTree::Delete(KEY_TYPE data)
{
    ...
    if (pBrother->GetCount() > ORDER_V)
    {
        ...
        pOldNode->Insert(new Tuple(NewData,TEMP));
